...
Needs: fonttools, brotli.   Output: two faces of one family split by code
point (FACES below) — fonts/MapleMonoCN-subset-latin.woff2 and
fonts/MapleMonoCN-subset-cjk.woff2 — plus fonts/coverage.txt, the subset's
actual code points and each face's hash: the sidecar that lets
`test/font-coverage.test.ts` hold the committed artifacts against the recipe
and the demo content without a font stack. Commit the four files together.
"""
...
# a fixed timestamp makes the woff2 a pure function of source + recipe
...
# Two faces of one family, split by code point, so a page downloads only the
# half its code uses: everything below the CJK blocks (ASCII, Latin, Greek,
# Cyrillic, punctuation, arrows, math, box drawing) and everything from
# U+2E80 up (CJK radicals and punctuation, kana, Hanzi, fullwidth forms).
# styles/tokens.css declares the same boundary as each face's unicode-range.
CJK_START = 0x2E80
FACES = {
    'MapleMonoCN-subset-latin.woff2': lambda cp: cp < CJK_START,
    'MapleMonoCN-subset-cjk.woff2': lambda cp: cp >= CJK_START,
}
COVERAGE = HERE / 'coverage.txt'
...
def main() -> None:
    ...
    chars = base | extra
    # `absent` lines in the committed coverage sidecar survive scanless runs
    coverage_path = COVERAGE
    carried: list[str] = []
    if coverage_path.is_file():
        for line in coverage_path.read_text(encoding='utf-8').splitlines():
            if line.startswith('absent '):
                carried.append(chr(int(line.split()[1], 16)))
    scan_absent = sorted(set(absent) | set(carried)) if args.scan else carried
    ...
    subset_cmap: list[int] = []
    hashes: list[str] = []
    for name, belongs in FACES.items():
        opts = subset.Options()
        opts.flavor = 'woff2'
        ...
        sub.populate(unicodes=[ord(c) for c in chars if belongs(ord(c))])
        sub.subset(face)
        face.flavor = 'woff2'
        out = HERE / name
        face.save(out)
        face_cmap = sorted(TTFont(out).getBestCmap())
        subset_cmap += face_cmap
        hashes.append(f'sha256 {name} {hashlib.sha256(out.read_bytes()).hexdigest()}')
        print(f'{name}: {len(face_cmap)} code points, {out.stat().st_size // 1024} KB (source {src})')
    subset_cmap.sort()
    lines = [
        '# Code points present in the MapleMonoCN-subset-*.woff2 faces, one U+XXXX per line.',
        '# Generated by build_font_subset.py alongside the woff2 files; commit together.',
    ...
    print(f'{coverage_path.name}: {len(subset_cmap)} code points across the faces')
