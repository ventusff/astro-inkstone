...
title: Kitchen sink
brand: Kitchen sink
subtitle: "every pipeline switch, once\nthe page is its own test bed"
description: Every element of the markdown pipeline performs once on this page — the page itself is their test bed.
kind: reference
domains: [pipeline]
tags: [markdown, math, mermaid, callouts]
status: evergreen
created: 2026-08-01
updated: 2026-08-22
aliases: [markdown-demo]
---
...
Emphasis parsing is CJK-friendly: **bold closes even against Chinese punctuation** — `**报文。**同时` renders as **报文。**同时, not as literal asterisks. *Italic*, ~~strikethrough~~ and `inline code` work as usual; with the `gemoji` switch a shortcode such as `:sparkles:` renders as :sparkles:. Characters inside inline code take part in no parsing, so backticks are the safe way to *show* syntax: `**`, `$…$` and `> [!note]` all appear literally. To show an asterisk in prose, escape it — \*like this\* stays plain.

## Wikilinks

With the `wikilinks` switch on, `[[double-bracket]]` links resolve against the note collection the way a wiki expects: by id ([[design-tokens]]), by alias ([[boundaries]] reaches the note whose id is `three-way-split`), with a label ([[getting-started|the getting-started guide]]), and with an anchor ([[checks#the-render-layer-probe]]). A target that resolves to nothing renders as a marked dead link instead of failing the build — and the engine's `check-wikilinks` CLI reports it in CI, where link rot belongs.
...
> The quote syntax serves plain Markdown and inbox-imported notes; the component serves MDX; raw HTML is the shared substrate beneath both — all three render the same `<aside class="callout …">` markup.
...
<div class="callout intuition">
  ...
  Teal left edge, for paragraphs about <em>how to think of it</em> rather than <em>what it is</em>.
</div>
