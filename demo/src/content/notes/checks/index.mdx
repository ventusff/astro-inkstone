...
Beyond compiling, it catches two frontmatter classes of silent loss: an unquoted `#` in a value (YAML reads it as a comment and silently truncates the rest) and YAML that does not parse at all, reported with its line.
...
```bash title="this garden's link check, as CI runs it"
node vendor/astro-inkbrush/scripts/check-wikilinks.mjs src/content/notes --locale-prefix zh/ --strict
```
...
```bash title="site root, after a build (pass --base when the site deploys under a subpath)"
